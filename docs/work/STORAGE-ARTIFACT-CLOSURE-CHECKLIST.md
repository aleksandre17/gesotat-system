...
- [x] 17.20 Page 11 declared includes — CLOSED (2026-09-20, `docs/evidence/page11-includes-serving-2026-09-20.json`): the contract now carries a serving policy (migration 109). `INPUT_FOR_CARRIER` is served to every reader from canonical raw storage of the published snapshot; `INPUT_RAW_LINEAGE` only to a caller holding `RAW_READ`, and it is not even advertised to anyone else. No table was re-labelled as DATA, keyset stays Data Plane only, and 114 tests plus a live run on dev prove both sides.
