...
- [ ] 10.1 Frontend `/files/...` → governed download — READY on the platform side (17.18): the site web server forwards API-issued signed URLs; the frontend code change itself (ask the API, rewrite the origin) is still to be made in the KIDS frontend (AIR-2026-009/013).
...
- [x] 14.2 Tenant/site/owner authorization is enforced from authenticated claims through dataset and artifact access — DONE on dev (17.19, ADR-010): product → tenant model, deny by default, declared and build-guarded controller coverage, no existence oracle, governed assignment. Open inside it: 12 legacy surfaces have no product identity and are recorded as tenant-neutral LEGACY (AIR-2026-044).
...
- [ ] 14.5 Rollback, replay, load/performance and recovery evidence — PARTIAL (17.17): load, recovery and concurrent replay proven on dev; rollback of a publication is not proven because nothing was published (owner decision), and backend capacity beyond the per-client rate limit was not measured.
...
- ~~EXT-4 `files.geostat.internal`~~ — CLOSED on dev (2026-09-19, 17.16): internal CA + dedicated leaf, edge redeployed, `STORAGE_PUBLIC_ENDPOINT=https://files.geostat.internal`. Clients still need the hosts line and the CA certificate once (edge README).
...
- [x] 17.12 Explicit relation-table match rule (17.8a, contract §4.2/§4.3/§27) — READY in source (AIR-2026-039): `RELATION_TABLE` rule; edges derived at admission, stored in the accepted manifest document and replayed at snapshot binding through the unchanged matcher. `RelationTableMatchRuleAcceptanceTest`: 1000 rows / 1500 files / 1502 edges identical at admission, in the document and at binding. No approved contract declares the rule yet, so there is no runtime run.
...
- [x] 17.15 SQL Server trigger fixture for migration 099 (`ops/tests/sql/site-contract-revision-governance.sql`, rollback-only) — PASS on dev: approved document immutable (51041), no return to review, no delete, second APPROVED rejected (51042), supersede-then-approve accepted, approval evidence append-only (51040), breaking approval needs an acknowledgement.
- [x] 17.16 Governed downloads over HTTPS (`https://files.geostat.internal`, EXT-4): the existing edge certificate is byte-identical (a re-issued self-signed certificate is NOT trusted by OpenSSL/curl/browser clients that hold the old one — proven and avoided, AIR-2026-040); internal CA (EC P-256, pathlen 0) + leaf for the download host only; `nginx -t` before one edge restart; Keycloak discovery still validates with the old anchor; production and dev health UP. Smoke: 200 with matching SHA-256 from the host and from inside `geostat-net`; without the CA → TLS refusal; unsigned, tampered, `POST`/`PUT` → 403. Verified twice (agent + lead, different files). Tool: `ops/scripts/shell/tls/issue-internal-ca-leaf.sh`. Evidence: `docs/evidence/files-edge-tls-runtime-2026-09-19.json`.
- [x] 17.17 Load, recovery and concurrent replay on dev (AIR-2026-041): 11 minutes, 1.05 M requests, 0 5xx, 0 transport errors; admission bounded by the rate limit (120 requests / 60 s per client, 99.9 % answered 429 with `RateLimit-Policy` and `Retry-After`); admitted-request latency p95 25 ms at baseline, 63 ms at 30 clients, 77 ms at the 60-client burst; production `/health` sampled every 10 s: 57 of 57 UP, 11–44 ms. Restart recovery: healthy in 27 s, package run 2 still `COMPLETED`, manifest document and download 200 on the first attempt. 10 simultaneous starts of manifest 7 → 10 × 202 with the same run id, no second run. Harness: `ops/tests/load/`. Evidence: `docs/evidence/storage-line-load-and-recovery-runtime-2026-09-19.json`.
- [x] 17.18 Same-host site delivery (AIR-2026-042): the KIDS frontend will run on the platform server, so the browser of a visitor must not need the internal hostname or CA. `ops/compose/projects/geostat/services/web/governed-files-proxy.conf` lets the site web server forward signed GET/HEAD to the internal download endpoint with upstream TLS verification against the internal CA. Proven on the server with a throwaway nginx on `geostat-net`: 200 + matching SHA-256 through the proxy, unsigned → 403, `POST` → 403; nothing published, containers removed (`ops/tests/edge/governed-files-proxy-smoke.sh`). The host itself has no passwordless sudo, so `/etc/hosts` and the system CA store were not changed and are not needed for this pattern.
- [x] 17.19 Security acceptance on dev (AIR-2026-043): tenant-scoped authorization (migration 104 `platform.data_product.tenant_key` + append-only assignment history; `TenantAccessPolicy`, `CurrentCaller` with an explicit SYSTEM caller for workers, `TenantScopeResolver` strategies, `@TenantScoped`/`@TenantNeutral` on every controller, `TenantScopeCoverageTest` fails the build on an unclassified controller; `PUT /platform/products/{code}/tenant`). 401/403 are RFC 9457 with the RFC 6750 `WWW-Authenticate` challenge from both the filter chain and method security; clock skew is explicit and bounded (`OIDC_CLOCK_SKEW_SECONDS`, 0..120). Runtime suite `ops/tests/security/` (site-agnostic, throwaway identity-provider clients created and deleted): run 3 = 57 of 60 PASS — forged/`alg=none`/algorithm-confusion/expired/wrong issuer/wrong audience/missing or blank tenant → 401; missing authority → 403; own-tenant ordinary identity → 200; foreign tenant → 404 masked on distribution (bodies equal to an unknown object) and 403 on contract, page and id-addressed routes. Not PASS: page 11 declared includes (400, see 17.20), one declared-tolerance case (token accepted 5 s after expiry, refused at 75 s), empty `Bearer` challenge shape (fixed in source after the run, unit-tested). `:api:test` 340, `:core:test` 30. Evidence: `docs/evidence/security-acceptance-runtime-2026-09-19.json`.
- [ ] 17.20 Page 11 declared includes return 400 — DECISION RECORDED, implementation open (AIR-2026-045): in revision 8 only 4 of 15 table definitions are `storage_plane=DATA`; both include targets (`KIDS_STATISTICAL_CARRIER`, `KIDS_RAW_DOCUMENT`) are `ACCESS`. Decision by data-minimisation (GDPR Art. 25, OWASP API3): raw source documents are not served to page consumers; `INPUT_RAW_LINEAGE` is to be served only to callers holding the raw-read authority, `INPUT_FOR_CARRIER` to every reader. Needs a RAW-family serving adapter bound to canonical storage and a contract revision; not a toggle.
