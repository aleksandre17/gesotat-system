...
# Compose can contain infrastructure services (currently MinIO) that have no
# Gradle module, Dockerfile or application JAR. They still get a generated
# per-service compose file and are started by the normal health-check flow.
is_application_service() {
    [ -f "$SOURCE_ROOT/$1/build.gradle" ] && [ -f "$SOURCE_ROOT/$1/Dockerfile" ]
}
...
COMPOSE_FILE="ops/compose/projects/geostat/docker-compose.${ENVIRONMENT}.yml"
if [ -n "${GEOSTAT_ENV_FILE:-}" ]; then
    ENV_FILE="$GEOSTAT_ENV_FILE"
elif [ "$ENVIRONMENT" = "prod" ]; then
    ENV_FILE="ops/config/projects/geostat/shared/runtime/production.env"
else
    ENV_FILE="ops/config/projects/geostat/shared/runtime/local.env"
fi
...
# Every image must carry an immutable source identity.  Callers may override
# this with an annotated release tag, otherwise use the current commit (and
# fail closed when the checkout cannot provide one).
if [ -z "${IMAGE_REVISION:-}" ]; then
    IMAGE_REVISION="$(git describe --tags --always --dirty 2>/dev/null || true)"
fi
if [ -z "$IMAGE_REVISION" ] || [ "$IMAGE_REVISION" = "unknown" ]; then
    echo "  ERROR: IMAGE_REVISION cannot be determined; refusing provenance-less deploy"
    exit 1
fi
if [ "$ENVIRONMENT" = "prod" ] && [ -n "$(git status --porcelain 2>/dev/null)" ]; then
    echo "  ERROR: production deploy requires a clean checkout (git status --porcelain is non-empty)"
    exit 1
fi
...
if [ "$SKIP_BUILD" = "0" ]; then
    if [ "$SERVICE" = "all" ]; then
        active_modules=$(for s in "${SERVICES[@]}"; do is_application_service "$s" && echo "$s"; done | paste -sd, -)
    else
        active_modules=""
        is_application_service "$SERVICE" && active_modules="$SERVICE"
    fi
    ...
    [ -n "$active_modules" ] && GRADLE_PROPS="-PactiveModules=$active_modules"
fi
...
if [ "$SKIP_BUILD" = "1" ]; then
    ...
    for s in "${SERVICES[@]}"; do
        ...
        if ! is_application_service "$s"; then
            log "  [SKIP] $s is infrastructure (no Gradle build)"
            continue
        fi
        ...
        (cd "$SOURCE_ROOT" && ./gradlew ":$s:bootJar" -x test --no-daemon $GRADLE_PROPS) 2>&1 | tee -a "$local_build_log"
...
for s in "${SERVICES[@]}"; do
    [ "$SERVICE" != "all" ] && [ "$SERVICE" != "$s" ] && continue
    jar=$(find "$SOURCE_ROOT/$s/build/libs/" -name "*-boot.jar" 2>/dev/null | head -1)
    [ -z "$jar" ] && jar=$(find "$SOURCE_ROOT/$s/build/libs/" -name "*.jar" 2>/dev/null | grep -iv plain | head -1)
    if [ -z "$jar" ]; then
        log "  [WARN] No jar for $s — skipping"
        continue
    fi
    cp "$jar" "$SOURCE_ROOT/$s/app.jar"
...
upload_service() {
...

    {
        echo "=== Upload [$s] $(date) ==="
        if is_application_service "$s"; then
            scp "$SOURCE_ROOT/$s/Dockerfile" "$SERVER:$REMOTE/$s/" 2>&1
            scp "$SOURCE_ROOT/$s/app.jar"    "$SERVER:$REMOTE/$s/" 2>&1
        else
    ...
    if [ "$ENVIRONMENT" = "prod" ] && is_application_service "$s"; then
        ssh -n "$SERVER" "
    ...
    for pattern in $CRED_PATTERNS; do
        for f in "$SOURCE_ROOT/$s/"$pattern; do
            [ -f "$f" ] || continue
