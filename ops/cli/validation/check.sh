...
SOURCE_ROOT="$PROJECT_DIR/platform/apps/geostat/backend"
ERRORS=0
WARNINGS=0
...
if [ -f "$PROJECT_DIR/ops/compose/projects/geostat/docker-compose.prod.yml" ]; then
    ok "ops/compose/projects/geostat/docker-compose.prod.yml found"
else
    fail "ops/compose/projects/geostat/docker-compose.prod.yml missing"
fi

# canonical production environment
ENV_PROD="${GEOSTAT_ENV_FILE:-$PROJECT_DIR/ops/config/projects/geostat/shared/runtime/production.env}"
if [ -f "$ENV_PROD" ]; then
    ok ".env.prod found"
else
    fail "production env missing: set GEOSTAT_ENV_FILE to an external, uncommitted env file (template: ops/config/projects/geostat/shared/templates/common.env.example)"
fi

# gradlew
if [ "$SKIP_BUILD" = "0" ]; then
    if [ -f "$SOURCE_ROOT/gradlew" ] || [ -f "$SOURCE_ROOT/gradlew.bat" ]; then
        ok "gradlew found"
    else
...
mapfile -t SERVICES < <(
    ...
    "$PROJECT_DIR/ops/compose/projects/geostat/docker-compose.prod.yml" 2>/dev/null
)
is_infrastructure_service() {
    [ "$1" = "minio" ]
}
if [ ${#SERVICES[@]} -eq 0 ]; then
    fail "No services found in ops/compose/projects/geostat/docker-compose.prod.yml"
fi
...
for s in "${SERVICES[@]}"; do
    ...
    if is_infrastructure_service "$s"; then
        ok "$s is compose-managed infrastructure"
        continue
    fi
    ...
    # Dockerfile
    if [ -f "$SOURCE_ROOT/$s/Dockerfile" ]; then
    ...
    # build.gradle
    if [ -f "$SOURCE_ROOT/$s/build.gradle" ]; then
    ...
    if [ "$SKIP_BUILD" = "1" ]; then
        jar=$(ls "$SOURCE_ROOT/$s/build/libs/"*-boot.jar 2>/dev/null | head -1)
        [ -z "$jar" ] && jar=$(ls "$SOURCE_ROOT/$s/build/libs/"*.jar 2>/dev/null | grep -iv plain | head -1)
        if [ -n "$jar" ]; then
...
if [ -f "$ENV_PROD" ]; then
    ...
    check_var "DB_DATA_URL"       "required"
    check_var "DB_DATA_USER"      "required"
    check_var "DB_DATA_PASS"      "required"
    check_var "DB_ARCHIVE_URL"    "required"
    check_var "DB_ARCHIVE_USER"   "required"
    check_var "DB_ARCHIVE_PASS"   "required"
    check_var "STORAGE_ENDPOINT"  "required"
    check_var "STORAGE_ACCESS_KEY" "required"
    check_var "STORAGE_SECRET_KEY" "required"
    check_var "KIDS_SQL_PASSWORD" "required"
    check_var "JWT_SECRET" "required"
    check_var "PLATFORM_CURSOR_SIGNING_SECRET" "required"
    check_var "PLATFORM_RATE_LIMIT_REQUESTS" "required"
    check_var "PLATFORM_RATE_LIMIT_WINDOW_MS" "required"
    check_var "PLATFORM_RATE_LIMIT_MAX_CLIENT_KEYS" "required"
    check_var "PLATFORM_CORS_ALLOWED_ORIGINS" "required"

    # JWT secret length check
    jwt_secret=$(grep -E "^JWT_SECRET=" "$ENV_PROD" | cut -d= -f2- | tr -d '"' | tr -d "'")
    if [ -n "$jwt_secret" ] && ! echo "$jwt_secret" | grep -qiE "CHANGE_ME"; then
        jwt_len=${#jwt_secret}
        if [ "$jwt_len" -lt 64 ]; then
            fail "JWT_SECRET too short (${jwt_len} chars, minimum 64 required)"
        else
            ok "JWT_SECRET length OK (${jwt_len} chars)"
        fi
    fi

    cursor_secret=$(grep -E "^PLATFORM_CURSOR_SIGNING_SECRET=" "$ENV_PROD" | cut -d= -f2- | tr -d '"' | tr -d "'")
    if [ -n "$cursor_secret" ] && [ ${#cursor_secret} -lt 32 ]; then
        fail "PLATFORM_CURSOR_SIGNING_SECRET too short (minimum 32 required)"
    fi

    cors_origins=$(grep -E "^PLATFORM_CORS_ALLOWED_ORIGINS=" "$ENV_PROD" | cut -d= -f2-)
    cors_patterns=$(grep -E "^PLATFORM_CORS_ALLOWED_ORIGIN_PATTERNS=" "$ENV_PROD" | cut -d= -f2-)
    if echo "$cors_origins,$cors_patterns" | grep -q '\*'; then
        fail "Wildcard CORS origins/patterns are forbidden in production"
    fi
...
for s in "${SERVICES[@]}"; do
    ...
    if ssh -n "$SERVER" "[ -d $REMOTE/$s ] && echo ok || echo missing" 2>/dev/null | grep -q "ok"; then
        ...
        # ops/compose/projects/geostat/docker-compose.prod.yml on server
