...
set PROJECT_DIR=%~dp0..
set COMPOSE_FILE=%PROJECT_DIR%\ops/compose/projects/geostat/docker-compose.prod.yml
set CHECK_SSH=0
set CHECK_BUILD=0
...
:: ──────────────────────────────────────────────
:: [3] ops/compose/projects/geostat/docker-compose.prod.yml exists and valid?
:: ──────────────────────────────────────────────
echo  [CHECK] ops/compose/projects/geostat/docker-compose.prod.yml...
...
if not "!PYTHON!"=="" (
    ...
    if !ERRORLEVEL! NEQ 0 (
        echo         [FAIL] YAML parse error in ops/compose/projects/geostat/docker-compose.prod.yml
        set /a FAIL+=1
    ) else (
...
if exist "%PROJECT_DIR%\ops\config\secrets\production\.env.prod" (
    echo         [OK]   .env.prod found
    set /a PASS+=1
) else if exist "%PROJECT_DIR%\ops\config\templates\common.env.example" (
    echo         [WARN]  production env missing ^(use ops\config\templates\common.env.example^)
    set /a WARN+=1
) else (
