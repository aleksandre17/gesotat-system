...
$ROOT    = Join-Path $REPO_ROOT 'platform/apps/geostat/frontend/geostat-system-app'
$CONFIG_ROOT = Join-Path $REPO_ROOT 'ops/config/projects/geostat/services/control-plane-ui'
$COMPOSE_ROOT = Join-Path $REPO_ROOT 'ops/compose/projects/geostat/services/control-plane-ui'
...
function Get-EnvValue {
    ...
    foreach ($envFile in @("local", "development")) {
        $path = Join-Path $CONFIG_ROOT "secrets/$envFile"
...
function Get-DockerServices {
    $composeFile = Join-Path $COMPOSE_ROOT "docker-compose.yml"
