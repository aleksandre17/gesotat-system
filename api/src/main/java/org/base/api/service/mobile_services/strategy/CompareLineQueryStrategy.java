...
import org.base.api.service.mobile_services.arcitecture.*;
...
//        addGroupBy(config1Query, p, true);
    ...
    private void addGroupBy(QueryBuilder query, CompareParams p, boolean isConfig1) {
        if (isConfig1) {
            if (p.getBrand1() != null) query.addGroupBy("t.brand");
            if (p.getModel1() != null) query.addGroupBy("t.model");
            if (p.getYearOfProduction1() != null) query.addGroupBy("t.year_of_production");
        } else {
            if (p.getBrand2() != null) query.addGroupBy("t.brand");
            if (p.getModel2() != null) query.addGroupBy("t.model");
            if (p.getYearOfProduction2() != null) query.addGroupBy("t.year_of_production");
        }
    }
