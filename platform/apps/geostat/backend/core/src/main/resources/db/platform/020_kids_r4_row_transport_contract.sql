/* Revision 4 corrects raw-vs-transport terminology: generated row JSON is a named transport snapshot, never raw payload. */
...
SELECT @contract,s.source_locator,'ACCESS_TABLE',v.dataset_version_id,s.source_key,s.row_role,s.load_order,
 CONCAT(N'{"approvalState":"DRAFT","canonicalAccessRevision":4,"datasetCode":"',s.dataset_code,N'","rawLineageRequired":true,"rowTransportEncoding":"ACCESS_ROW_CANONICAL_JSON_V1"}'),1
...
UPDATE platform.ingestion_contract SET contract_revision=4,status='REVIEW_REQUIRED',auto_publish=0,raw_ingest_enabled=1 WHERE contract_id=@contract;
