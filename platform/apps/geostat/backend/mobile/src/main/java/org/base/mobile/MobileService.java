...
//    )
    ...
    private YearRange fetchYearRange(StackedQueryStrategy strategy) {
        ...
        return new YearRange(minYear, 2025);
    }
...
//        }
        ...
        // Anchor the window to the most recent year (bounded by the query's WHERE t.year != 2026) and show the
        // last getYearRange() years, e.g. 2018–2025. This keeps the newest year in view instead of the old bug,
        // which anchored to the absolute minYear and cut the recent years off the end.
        int maxYear = results.stream()
                .filter(r -> r.get("year") != null)
                .mapToInt(r -> ((Number) r.get("year")).intValue())
                .max().orElse(minYear);
        int windowStart = Math.max(minYear, maxYear - strategy.getYearRange() + 1);
        int range = maxYear - windowStart + 1;

        // Process results
        Map<String, TradeDTO.CountryData> countryMap = new LinkedHashMap<>();
        for (Map<String, Object> row : results) {
            String name = row.get("name") != null ? (String) row.get("name") : "Unknown";
            int year = row.get("year") != null ? ((Number) row.get("year")).intValue() : windowStart;
            long value = row.get("value") instanceof Number ? ((Number) row.get("value")).longValue() : 0L;

            if (year < windowStart || year > maxYear) continue;

            countryMap.computeIfAbsent(name, k -> new TradeDTO.CountryData(
                    k, new ArrayList<>(Collections.nCopies(range, 0L))
            )).data().set(year - windowStart, value);
        }
        ...
        return new TradeDTO(windowStart, topCountries);
    }
...
//        }
   // }
    ...
    public FuelColumnDTO getFuelColumn(FuelColumnParams params, String tableName, @Valid @NotBlank String langName) {
        ...
        // Determine each year's own top-5 countries by that year's value (the top 5 can differ year to year)
        Map<Integer, Set<String>> topCountriesByYear = results.stream()
                .filter(row -> row.get("country_name") != null)
                .collect(Collectors.groupingBy(row -> ((Number) row.get("year")).intValue()))
                .entrySet().stream()
                .collect(Collectors.toMap(Map.Entry::getKey, entry -> entry.getValue().stream()
                        .sorted(Comparator.comparingDouble((Map<String, Object> row) -> row.get("currency") instanceof Number
                                ? ((Number) row.get("currency")).doubleValue() : 0.0).reversed())
                        .limit(5)
                        .map(row -> (String) row.get("country_name"))
                        .collect(Collectors.toSet())));

        // Build series map, keeping only each year's top-5 countries
        ...
        for (Map<String, Object> row : results) {
            ...
            int year = ((Number) row.get("year")).intValue();
            if (!topCountriesByYear.getOrDefault(year, Set.of()).contains(countryName)) continue;
            double currencyValue = row.get("currency") instanceof Number ?
