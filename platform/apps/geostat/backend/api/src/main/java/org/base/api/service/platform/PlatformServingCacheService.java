...
/** Generic, contract-independent rebuild of the published metric serving read model. */
@Service
public class PlatformServingCacheService {
    ...
    public void rebuildPublished() {
        ...
        try { data.update("INSERT serving.metric_cache(snapshot_id,metric_id,dimension_signature,period_start,period_end,aggregate_value) " +
            ...
            // Scheduler must remain alive and retry on the next tick; avoid dumping credentials/SQL stack traces.
            log.warn("Serving-cache rebuild deferred because the data plane is unavailable: {}", ex.getMostSpecificCause().getMessage());
        }
    }
}
